...
export class CdkStage extends CoreConstruct {
  ...
  public addApplication(appStage: Stage, options: AddStageOptions = {}) {
    ...
    for (const stacks of sortedTranches) {
      ...
      for (const stack of stacks) {
        this.addStackArtifactDeployment(stack, { runOrder, executeRunOrder, parameterOverrides: options.parameterOverrides });
      }
    }
  }
  ...
  private prepareStage() {
    ...
    for (const { prepareRunOrder, stackArtifact, executeRunOrder, parameterOverrides } of this.stacksToDeploy) {
